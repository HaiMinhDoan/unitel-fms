...
<script lang="ts" setup>
  ...
  import { createDriverDocument, updateDriverDocument } from '@/api/fms/driver';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    currentDriverId.value = data?.driverId || '';
  ...
  const getTitle = computed(() => (!unref(isUpdate) ? 'Thêm hồ sơ tài xế' : 'Sửa hồ sơ tài xế'));
  ...
  async function handleSubmit() {
    try {
      ...
      if (unref(isUpdate)) {
        await updateDriverDocument(currentId.value, params);
      } else {
        await createDriverDocument(params);
      }
