...
<script lang="ts" setup>
  ...
  import { filterPartners } from '@/api/fms/partner';
  import PartnerModal from './PartnerModal.vue';
  import { columns, searchFormSchema } from './partner.data';

  const [registerModal, { openModal }] = useModal(); 
  const { t } = useI18n();

  const [registerTable, { reload }] = useTable({
    title: t('fms.partners.title'),
    api: async (params) => {
      const filters: any[] = [];
      ...
      const res = await filterPartners(requestParams);
