...
public class DriverController {
    ...
    public ResponseData<DriverResponse> create(@Valid @RequestBody DriverRequest request) {
        ...
        return ResponseData.<DriverResponse>builder().status(200).messageCode("SUCCESS").data(driverMapper.toResponse(saved)).build();
    }
    ...
    public ResponseData<DriverResponse> getById(@PathVariable UUID id) {
        ...
        if (entity == null) {
            return ResponseData.<DriverResponse>builder().status(404).messageCode("NOT_FOUND").build();
        }
        return ResponseData.<DriverResponse>builder().status(200).messageCode("SUCCESS").data(driverMapper.toResponse(entity)).build();
    }
    ...
    public ResponseData<List<DriverResponse>> getAvailableForDispatch() {
        ...
        return ResponseData.<List<DriverResponse>>builder().status(200).messageCode("SUCCESS").data(list).build();
    }
    ...
    public ResponseData<DriverEligibilityResult> checkDispatchEligibility(@PathVariable UUID id) {
        ...
        return ResponseData.<DriverEligibilityResult>builder().status(200).messageCode("SUCCESS").data(result).build();
    }
    ...
    public ResponseData<DriverResponse> getMe() {
        ...
        return ResponseData.<DriverResponse>builder().status(200).data(driverMapper.toResponse(driver)).messageCode("SUCCESS").build();
    }
}
